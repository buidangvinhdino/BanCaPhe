package com.example.bancaphe.DAOModel;
...
import com.example.bancaphe.Model.LuuHoaDon;
...
public class DAOLuuHD {
    ...
    public ArrayList<LuuHoaDon> tkNhanVien(){
        ...
        Cursor cursor= database.rawQuery("Select User.MaUser, User.fullname,User.username,User.Chucvu,User.ChucVu,User.SDT,User.namsinh,SUM(LuuHoaDon.soluong * LuuHoaDon.dongia) as doanhThu From User left join LuuHoaDon on User.MaUser= LuuHoaDon.mauser GROUP By User.MaUser", null);
        if (cursor.getCount() !=0){
            ...
            do {
                ...
                double tt = cursor.getDouble(7);
                listTKNV.add(new LuuHoaDon(maUser,fullName,userName,chucvu,userSDT,userNamSinh,userDoanhThu,tt));
...
//    Lấy doanh thu theo khoảng thời gian
    ...
    public  ArrayList<LuuHoaDon> getAllHoaDon(int caseTK, int maUserInput){
        ...
        if (caseTK==2){
            ...
            cursor = database.rawQuery("Select LuuHoaDon.maLuu, LuuHoaDon.mahoadon,LuuHoaDon.tenkhachhang, sum(LuuHoaDon.soluong * LuuHoaDon.dongia) From LuuHoaDon where LuuHoaDon.maUser=? Group by LuuHoaDon.maHoaDon;", new String[]{String.valueOf(maUserInput)});
        }
        if (cursor.getCount() !=0){
            cursor.moveToFirst();
            do {
